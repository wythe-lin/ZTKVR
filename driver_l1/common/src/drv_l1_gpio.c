...
BOOLEAN gpio_init_io(INT32U port, BOOLEAN direction)
{
    ...
    if((port >= IO_I0) && (port <= IO_I9)){
	    if (direction == GPIO_OUTPUT) {
	        DRV_Reg32(IOI_BUFFER_ADDR) |= (1 << gpio_set_num);  /*Set attribute to 0 for input */
	        DRV_Reg32(IOI_DIR_ADDR) |= (1 << gpio_set_num);
	    }
	    else if (direction == GPIO_INPUT) {
	        DRV_Reg32(IOI_BUFFER_ADDR) &= ~(1 << gpio_set_num);  /*Set attribute to 1 for output */
	        DRV_Reg32(IOI_DIR_ADDR) &= ~(1 << gpio_set_num);
	    }
    }else{
    	if (direction == GPIO_OUTPUT) {
