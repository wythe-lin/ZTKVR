...
INT32S ap_state_resource_init(void)
{
	...
	ap_state_resource_time_stamp_position_x_set(10/*40*/);		//wwj modify
	ap_state_resource_time_stamp_position_y_set(440/*430*/);	//wwj modify
	return STATUS_OK;
}
...
INT16U ap_state_resource_ir_key_num_get(void)
{
	if (!resource_header) {
		return 0;
	}
...
INT32S ap_state_resource_ir_table_load(void)
{
	//if no ir key, return fail
	if (!resource_header || resource_header->irkey_num == 0) {
		return STATUS_FAIL;
	}
	gp_memcpy((INT8S*)ir_key_code,(INT8S*)keycode,resource_header->irkey_num);
	gp_memcpy((INT8S*)ir_key_msg_id,(INT8S*)eventid,resource_header->irkey_num*4);
...
void ap_state_resource_ir_key_map_get(INT8U **keycode, INT32U **key_msg)
{
	*keycode = ir_key_code;
