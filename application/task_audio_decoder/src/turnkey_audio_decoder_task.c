...
static void audio_init(void)
{
	...
	gp_memset(audio_ctrl.work_mem,0,MP3_DEC_MEMORY_SIZE);
...
extern void ap_state_handling_icon_show_cmd(INT8U cmd1, INT8U cmd2, INT8U cmd3); //wwj test
...
INT32S audio_mem_alloc(INT32U audio_format,AUDIO_CTRL *aud_ctrl, INT16S *pcm[])
{
	...
	//}
	for (i=0;i<dac_buf_nums;i++) {
		pcm[i] = (INT16S *) (pcm_buf + (i*pcm_size*2));
